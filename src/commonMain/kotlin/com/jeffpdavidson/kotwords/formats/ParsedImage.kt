...
internal class ParsedImage private constructor(private val coreImage: CoreImage32) {
    ...
    fun crop(
        ...
        (0 until height).forEach { row ->
            // Calculate the starting position in the original flat array
            // (y + row) selects the line, then we add x to reach the column
            val sourceRowStart = (y + row) * this.width + x
            val sourceRowEnd = sourceRowStart + width

            // Calculate where this row starts in the new flat array
            val destinationRowOffset = row * width

            // Efficiently copy the slice of the row
